...
class OnPolicyActorBuffer:
    ...
    def __init__(self, args, obs_space, act_space):
        ...
        if act_space.__class__.__name__ == "Discrete":
            ...
            # For MultiDiscrete, the total number of actions is the product of dimensions
            num_actions = np.prod(act_space.nvec)
            self.available_actions = np.ones(
                (self.episode_length + 1, self.n_rollout_threads, num_actions),
                dtype=np.float32,
            )
        else:
